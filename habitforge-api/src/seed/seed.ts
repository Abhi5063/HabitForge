/**
 * HabitForge Database Seed Script
 *
 * Seeds:
 *   - 10 dummy leaderboard users (User table)
 *   - 10 badge definitions (Badge table)
 *
 * Run with:
...
const DUMMY_LEADERBOARD_USERS: LeaderboardUser[] = [
...
async function seedBadges(): Promise<void> {
  ...
  for (const badge of BADGE_DEFINITIONS) {
    await prisma.badge.upsert({
      where: { badgeKey: badge.id },
      update: {
        name: badge.name,
        description: badge.desc,
        icon: badge.icon,
        rarity: badge.rarity,
        criteriaType: badge.criteria,
        criteriaValue: badge.value,
      },
      create: {
        badgeKey: badge.id,
        name: badge.name,
...
async function seedLeaderboardUsers(): Promise<void> {
  console.log('👤 Seeding dummy leaderboard users...')

  const passwordHash = await bcrypt.hash('HabitForge2024!', 12)

  for (const u of DUMMY_LEADERBOARD_USERS) {
    const username = toUsernameSlug(u.name)
    const email = `${username}@habitforge.dummy`
  ...
  console.log(`  ✓ Seeded ${DUMMY_LEADERBOARD_USERS.length} dummy users`)
}

// ── Main ──────────────────────────────────────────────────────────

async function main(): Promise<void> {
  console.log('\n🚀 HabitForge seed starting...\n')
